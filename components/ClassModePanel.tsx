...
const SeatMatrix = ({ session, user }: { session: any, user: any }) => {
  ...
  const fetchSeatMap = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/class-mode/seats?session_id=${session.id}`);
      const data = await res.json();
      if (data.error) throw new Error(data.error);
      setSeatMap(data.seatMap || []);
      
      // Check if the current user already has a seat
      const mySeat = data.seatMap.find((s: any) => String(s.user_id) === String(user.id));
      if (mySeat) {
    ...
    try {
      const res = await fetch('/api/class-mode/seats', {
        ...
        body: JSON.stringify({
          ...
          seat_row: selectedSeat.y,
          seat_col: selectedSeat.x,
          account_id: user.id,
        }),
      });
      const data = await res.json();
  ...
  const renderSeat = (x: number, y: number) => {
    ...
    if (seatData) {
      if (seatData.user_id === user.id) { // My seat
        content = (
          <>
  ...
  const renderGrid = () => {
    ...
    for (let y = 0; y < rows; y++) {
      ...
      for (let x = 0; x < cols; x++) {
        // Aisle logic: 2-4-2 (seats 0-1, aisle, seats 3-6, aisle, seats 8-9)
        if (x === 2 || x === 7) {
           row.push(<div key={`aisle-${y}-${x}`} className="w-full h-20"></div>); // Aisle
        } else {
...
export default function ClassModePanel({ user }: { user: any }) {
  ...
  const handleStartClassSession = async () => {
    ...
    setLoading(true);
    try {
      // Find the selected class name to use in the session name
      ...
      const res = await fetch('/api/class-mode/sessions', {
        ...
        body: JSON.stringify({ 
          ...
          title: sessionName // Corrected: send 'title' instead of 'name'
        }),
      });
